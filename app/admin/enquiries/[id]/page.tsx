...
export default async function EnquiryDetailPage({ params }: EnquiryDetailPageProps) {
  ...
  const YesNoBadge = ({ value }: { value: boolean | null }) => {
    ...
    return value ? (
      ...
      <Badge variant="outline" className="border-slate-200 bg-slate-50 text-slate-600 gap-1">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <Badge variant="outline" className={`${getStatusColor(enquiry.status)} text-sm px-3 py-1`}>
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <Card className="border-0 bg-gradient-to-br from-emerald-600 to-emerald-700 text-white shadow-lg overflow-hidden">
            <CardContent className="p-8">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-emerald-100 text-sm font-medium uppercase tracking-wider">Total Quote</p>
                  <p className="text-5xl font-bold mt-2">{formatCurrency(enquiry.quote_amount)}</p>
            ...
            <CardContent className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="mt-1.5 text-slate-900 font-medium text-lg">{enquiry.first_name} {enquiry.last_name}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="mt-1.5 flex items-center gap-2">
                    ...
                    <a href={`mailto:${enquiry.email}`} className="text-emerald-600 hover:underline font-medium">
                      {enquiry.email}
                    </a>
                  </div>
                </div>
                <div>
                  ...
                  <div className="mt-1.5 flex items-center gap-2">
                    ...
                    <a href={`tel:${enquiry.phone}`} className="text-slate-900 font-medium">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="md:col-span-2 lg:col-span-3">
                  ...
                  <div className="mt-1.5 flex items-start gap-2">
                    ...
                    <p className="text-slate-900 font-medium">{enquiry.property_address || "Address not provided yet"}</p>
                  </div>
                </div>
                <div>
                  <label className="text-xs font-medium text-slate-500 uppercase tracking-wider">Postcode</label>
                  <p className="mt-1.5 text-slate-900 font-mono font-semibold text-lg">{enquiry.property_postcode || "—"}</p>
                </div>
                <div>
                  <label className="text-xs font-medium text-slate-500 uppercase tracking-wider">Property Value</label>
                  <p className="mt-1.5 text-slate-900 font-semibold text-lg">
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-slate-900 font-medium">{getTenureLabel(enquiry.tenure)}</p>
                </div>
                <div>
                  <label className="text-xs font-medium text-slate-500 uppercase tracking-wider">Number of Owners</label>
                  <p className="mt-1.5 text-slate-900 font-medium">{enquiry.owner_count || "—"}</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-6">
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-slate-100">
                <div className="px-6 py-3 flex justify-between items-center">
                  ...
                  <span className="font-medium">{formatCurrency(enquiry.legal_fee)}</span>
                </div>
                {enquiry.leasehold_supplement > 0 && (
                  <div className="px-6 py-3 flex justify-between items-center">
                    <span className="text-slate-600">Leasehold Supplement</span>
                    <span className="font-medium">{formatCurrency(enquiry.leasehold_supplement)}</span>
                  </div>
                )}
                {enquiry.mortgage_fee > 0 && (
                  <div className="px-6 py-3 flex justify-between items-center">
                    <span className="text-slate-600">Mortgage Work</span>
                    <span className="font-medium">{formatCurrency(enquiry.mortgage_fee)}</span>
                  </div>
                )}
                {enquiry.new_build_fee > 0 && (
                  <div className="px-6 py-3 flex justify-between items-center">
                    <span className="text-slate-600">New Build Supplement</span>
                    <span className="font-medium">{formatCurrency(enquiry.new_build_fee)}</span>
                  </div>
                )}
                {enquiry.company_fee > 0 && (
                  <div className="px-6 py-3 flex justify-between items-center">
                    <span className="text-slate-600">Company Purchase</span>
                    <span className="font-medium">{formatCurrency(enquiry.company_fee)}</span>
                  </div>
                )}
                {enquiry.gift_funds_fee > 0 && (
                  <div className="px-6 py-3 flex justify-between items-center">
                    <span className="text-slate-600">Gift Funds Verification</span>
                    <span className="font-medium">{formatCurrency(enquiry.gift_funds_fee)}</span>
                  </div>
                )}
                <div className="px-6 py-3 flex justify-between items-center bg-slate-50/50">
                  <span className="text-slate-600">Subtotal</span>
                  <span className="font-medium">{formatCurrency(enquiry.subtotal)}</span>
                </div>
                <div className="px-6 py-3 flex justify-between items-center">
                  <span className="text-slate-600">VAT (20%)</span>
                  <span className="font-medium">{formatCurrency(enquiry.vat)}</span>
                </div>
                <div className="px-6 py-3 flex justify-between items-center">
                  <span className="text-slate-600">Disbursements</span>
                  <span className="font-medium">{formatCurrency(enquiry.disbursements)}</span>
                </div>
                <div className="px-6 py-4 flex justify-between items-center bg-emerald-50">
                  ...
                  <span className="font-bold text-lg text-emerald-900">{formatCurrency(enquiry.quote_amount)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-slate-200 shadow-sm">
            ...
            <CardContent className="p-4 space-y-2">
              ...
              <div className="pt-2">
                <Button className="w-full bg-emerald-600 hover:bg-emerald-700 justify-center gap-2" size="lg">
                  <UserPlus className="h-4 w-4" />
            ...
            <CardContent className="p-6">
              <div className="space-y-4">
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-sm text-slate-900 font-medium">Quote request submitted</p>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-sm text-slate-900 font-medium">Quote email sent to client</p>
                    <p className="text-xs text-slate-500">{formatDate(enquiry.created_at)} at {formatTime(enquiry.created_at)}</p>
                  </div>
                </div>
              </div>
